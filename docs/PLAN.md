...
`crates/corvid-ffi/corvid.h`, v0.3.3, 124 symbols), with the golden
...
> fixtures (267 executable lines across 8 files at v0.3.3) are the
...
v0.3.3 GitHub release, including the v0.3.0 additive OPs (VMAP_KEYS /
...
v0.3.3's corvid_page zero-length-cursor fix is dylib-behavioral with no
...
3. **Fetch + verify** — `fetch.sh` / `fetch.ps1` (founding pin v0.3.0,
   ...
   search; now v0.3.3, byte-identical fixtures).
...
5. **The golden port** — `test/golden.cpp` replays the v0.3.3 fixture
...
(`CORVID_VERSION=v0.3.3`). Bumps are a one-variable change plus a
